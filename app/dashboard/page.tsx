...
export default async function DashboardPage() {
  ...
  const { data: products } = await supabase?.from('products')?.select('*')?.eq('user_id', DEMO_USER_ID)?.order('expiration_date', { ascending: true })

  // Fetch unread notifications for demo user
  const { data: notifications } = await supabase?.from('notifications')?.select('*')?.eq('user_id', DEMO_USER_ID)?.eq('read', false)
